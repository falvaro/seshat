...
typedef multimap<string, boost::tuples::tuple<string, string, int, int> >::iterator WC_CONN_IT; 
typedef pair<string, boost::tuples::tuple<string, string, int, int> > WC_CONN_PAIR;
...
struct WeightContainer: public DataExporter
{
  ...
  multimap<string, boost::tuples::tuple<string, string, int, int> > connections;
  ...
  void link_layers(const string& fromName, const string& toName, const string& connName = "", int paramBegin = 0, int paramEnd = 0) {
    connections.insert(make_pair(toName, boost::tuples::make_tuple(fromName, connName, paramBegin, paramEnd)));
  }
